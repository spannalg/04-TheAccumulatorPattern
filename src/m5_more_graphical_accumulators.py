...
Authors: David Mutchler, Vibha Alangar, Matt Boutell, Dave Fisher, Mark Hays,
         ...
         Note: Received help from classmate James Werne and the Learning Center
...
def draw_squares_from_circle(n, circle, window):
    ...
    y = circle.center.y
    for k in range (n):
        square = rg.Square(rg.Point(x+(k*r),y+(r*k)), 2 * r)
...
def run_test_draw_circles_from_rectangle():
    ...
    ###########################################################################
    # -------------------------------------------------------------------------
    window1 = rg.RoseWindow(600,700)
    #Test 1
    rectangle = rg.Rectangle(rg.Point(150,150),rg.Point(200,200))
    draw_circles_from_rectangle(3,2,rectangle,window1)

    #Test 2
    rectangle = rg.Rectangle(rg.Point(500,600),rg.Point(600,700))
    draw_circles_from_rectangle(4,4,rectangle,window1)


    window2 = rg.RoseWindow(600,700)

    #Test 3
    rectangle = rg.Rectangle(rg.Point(400,450),rg.Point(550,650))
    draw_circles_from_rectangle(2,3,rectangle,window2)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range (m):
        ...
        circle =rg.Circle(rg.Point(x-((2*k+1)*(h/2)),y+(h/2)),(h/2))
        circle.attach_to(window)
        window.render()
    for k in range (n):
        l = rectangle.get_width()
        x = rectangle.get_upper_left_corner().x
        y = rectangle.get_upper_left_corner().y
        circle =rg.Circle(rg.Point(x+(l/2),y-((2*k+1)*(l/2))),(l/2))
        circle.attach_to(window)
        window.render()
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    window.render()
    delta_y = (rectangle1.get_height())/2
    delta_x = (rectangle1.get_width())/2
    for k in range(n):
        line = rg.Line(c1,c2)
        line.thickness = 5
        line.attach_to(window)
        c1 = rg.Point(c1.x - delta_x,c1.y + delta_y)
        c2 = rg.Point(c2.x - delta_x,c2.y + delta_y)
        if k%2 !=0:
